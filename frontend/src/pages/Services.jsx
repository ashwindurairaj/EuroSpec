              ...
              <Tabs defaultValue="tooling" className="w-full">
                <TabsList className="flex flex-wrap justify-center gap-2 mb-12 bg-transparent h-auto">
                  {services.map((s) => {
                    const Icon = s.icon
                    return (
                      <TabsTrigger key={s.id} value={s.id} data-testid={`service-tab-${s.id}`} className="flex items-center gap-2 px-6 py-3 data-[state=active]:bg-primary data-[state=active]:text-white rounded-md border border-gray-200 transition-colors">
                    ...
                    >
                      <div>
                        ...
                        <ul className="space-y-3 mb-8">
                          {s.features.map((f, i) => (
                            <motion.li key={i} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 + i * 0.08 }} className="flex items-start gap-3"><div className="w-2 h-2 bg-accent rounded-full mt-2 flex-shrink-0" /><span className="text-gray-700">{f}</span></motion.li>
                          ))}
                        </ul>
                        <Link to={`/services/${s.id}`} data-testid={`service-learnmore-${s.id}`} className="inline-flex items-center gap-2 text-accent font-semibold hover:gap-3 transition-all">Learn More <ArrowRight className="w-4 h-4" /></Link>
                      </div>
                      <motion.img initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} src={s.image} alt={s.title} className="w-full h-[400px] object-cover rounded-md shadow-lg" />
                    </motion.div>
                  </TabsContent>
                ))}
              </Tabs>
